...
|user_like| CockroachDB |
|artist_cache| Redis |
|track_cache| Redis |
|album_cache| Redis |
|document| Postgres, Amazon S3 |
| subscription| Postgres |
| user_playlist_like |CockroachDB |
|session |Redis|
|search_index| ElasticSearch |

Пользовательские таблицы, такие как user, user_like и user_playlist_like, хранятся с партиционированием по user_id, что позволяет обрабатывать запросы локально в регионе пользователя.
...
|artist| artist_id hash | 1 основной сервер и 2 реплики|
|album| album_id hash |1 основной сервер и 2 реплики|
...
|track| - | 1 основной сервер и 3 реплики|
...
- CockroachDB: Golang - github.com/cockroachdb/cockroach-go/v2, Rust - tokio-postgres

## Балансировка запросов
- PostgreSQL: PgBouncer 
- Redis: Redis-cluster
- CockroachDB: Каждый узел CockroachDB — это полноценный SQL-endpoint. Можно подключаться к любому узлу, и он сам перенаправит запрос к нужному реплике или лизинг-холдеру. Это даёт базовую балансировку: любой запрос можно послать на случайный узел, и кластер сам выберет оптимальный путь.

## Схема резервного копирования
- PostgreSQL: ежедневные снапшоты + wal архивирование
- Redis: RDB снапшоты.
- Amazon S3: внутренние методы amazon.
- CockroachDB: full backup раз в сутки, incremental backup раз в час.
...
Каждая композиция проходит через несколько этапов обработки, в результате которых формируется её уникальное математическое описание.

На первом этапе алгоритм получает цифровое представление звука — аудиосигнал, который можно записать как последовательность отсчётов x(t), где t - время, а x - амплитуда. Сначала сигнал разбивается на короткие временные окна (например, по 20–40 миллисекунд), к каждому из которых применяется Быстрое преобразование Фурье, это даёт возможность построить спектрограмму — график, показывающий, какие частоты звучат в каждый момент времени.

На основе спектра извлекаются низкоуровневые признаки, такие как спектральный центроид — «центр масс» частотного распределения, который можно вычислить как
$C = \frac{\sum_kf_{k} \cdot S(f_{k})}{\sum_kS(f_{k})}$, где $f_k$ - частота, а $S(f_k)$ - её амплитуда.
Если центроид велик, значит трек имеет яркое звучание с большим количеством высоких частот; если мал — звук мягкий и приглушённый.

Помимо спектрального анализа, система определяет темп (BPM), тональность (key), динамику и ритмические паттерны. Эти параметры помогают алгоритму понять, насколько песня энергична или спокойна. Для оценки темпа часто используется автокорреляция сигнала, которая позволяет выявить повторяющиеся ритмические структуры.

Следующий слой анализа — извлечение среднеуровневых признаков, которые описывают музыкальные аспекты: мелодию, гармонию, структуру трека. Нейронные сети обучаются на большом количестве песен, чтобы распознавать, где начинается куплет, где припев, как меняется настроение и плотность звука. Для этого используется анализ изменения спектра во времени и алгоритмы сегментации аудиопотока.

Самый высокий уровень — семантический анализ. Здесь применяются глубокие нейросети (deep learning), в частности сверточные (CNN) и рекуррентные (RNN) модели, которые воспринимают спектрограмму как изображение и «распознают» в ней паттерны, связанные с жанром, вокалом, инструментами и даже эмоциональным состоянием композиции. Например, одна свёртка может выделять ударные, другая — вокал, третья — фоновые гармонии. Эти признаки объединяются в вектор $v_{track} = [f_1, f_2, \dots, f_n]$.

Далее алгоритм ищет треки, наиболее близкие к этому треку по метрике схожести. Обычно используется косинусное сходство.
$cos\_similarity = \frac{a \cdot b}{||a||\space||b||}$
...
| CockroachDB| Хранение данных | Автоматическое шардирование и репликация, поддержка горизонтального масштабирования и мультирегиональных кластеров, сильная консистентность и отказоустойчивость |
...
| Milvus | Система рекомендаций | Векторная база данных с высокой скоростью поиска |
...
|-----------|------------------------|
| Резервирование ресурсов | Несколько серверов с 65 - 70% нагруженностью |
| Резервирование приложения | Кэширование данных, запуск несколько контейнеров приложений, Kubernetes автоматически переподнимает упавшие контейнеры |
| Резервирование PostgreSQL | Логическое и аппаратное (master–replica). Репликация 1 мастер + 2–3 реплики; ежедневные снапшоты + WAL архивирование |
| Резервирование CockroachDB| Встроенное мультирегиональное резервирование. Автоматическая репликация и консенсус (Raft), full backup 1×сутки, incremental 1×час |
| Резервирование Redis | Кластерное с репликацией (master–replica). Redis Cluster, RDB снапшоты |
| Резервирование S3 | Геораспределённое (S3 Cross-Region Replication). Файлы автоматически копируются в соседний регион |
| Резервирование ElasticSearch | Репликация шардов (1 primary + 3 replica per shard). Реплики автоматически берут на себя роль primary при сбое |
| Резервирование L3/L4 | Аппаратное (N+1). Keepalived + LVS (Virtual Server via Direct Routing). Несколько балансировщиков с неполной загруженностью |
| Резервирование L7 | Аппаратное и программное (N+1). На каждый ДЦ — N+1, проверка доступности health-check’ами|

Также сбор метрик и их визуализация в Grafana, настройка alerting в Grafana.
Периодическое проведение chaos testing.


# 10. Схема проекта
![scheme](/img/scheme.svg)
Используется API Gateway для балансировки запросов между микросервисами. В ElasticSearch хранятся копии треков из PostgreSQL. Перед походом в базу микросервисы сначала ходят в кэш.

# 11. Список серверов
| Компонент | RPS | Пиковый RPS (x2) | CPU | RAM | Описание |
|-|-|-|-|-|-|
| Auth service | 7000  | 14000 | 14 | 24 | Сервис в основном служит для проверки авторизации пользователя |
| Recommendation service | 1611 | 3222 | 4 | 4 | Сервис служит для отправки запросов ML модели |
| ML model | 1611 | 3222 | 65 (GPU) | 32 | Модель для поиска похожей музыки |
| User service | 4000 | 8000 | 16 | 32 | Выбор лайкнутых треков и плейлистов |
| Track service | 3000 | 3300 | 8 | 16 | Работа с песнями |
| Album service | 1500 | 3000 | 4 | 8  | Работа с альбомами |
| Artist service | 1500 | 3000 | 4 | 8 | Работа с артистами |
| Search service | 6444 | 12888 | 12 | 16 | Поиск по названиям треков |
| Subscription service | 0.836 | 1.672 | 4 | 8 | Сервис для проверки подписки |
| Payment service | 0.836 | 1.672 | 4 | 8 | Сервис для оплаты подписки |
| Kafka | 1611 | 3222 | 4 | 16 | Отправка запросов к ML модели |
| Redis | 14000 | 28000 | 32 | 512 | Хранение кэшей, сессий |
| PostgreSQL | 5000 | 10000 | 32 | 256 | Основное хранилище, сложные запросы |
| Milvus | 900 | 1800 | 8 | 32 | Работа с векторным представлением пользователя |
| ElasticSearch | 6444 | 12888 | 32 | 512 | Хранение треков, а также векторизированный поиск |
| Prometheus + Grafana | 200 | 400 | 4 | 16 | Отправка уведомлений о метриках, наблюдение за метриками |
| L4 | 18500 | 37000 | 32 | 16 | Глобальная балансировка | 
| L7 | 12000 | 24000 | 32 | 64 | HTTP/HTTPS балансировка, SSL termination. Высокая нагрузка, кэширование, маршрутизация |

## Хостинг
Гибридный хостинг: совместное использование виртуальных машин и kubernetes.

Bare metal:
| Сервис        | Конфигурация                                   | vCPU на узел | RAM на узел (GB) | Кол-во | Итог vCPU | Итог RAM (GB) |
|---------------|------------------------------------------------|--------------|------------------|--------|-----------|----------------|
| PostgreSQL    | 2×6338 / 16×64GB / 4×NVMe64T / 2×25Gb/s        | 128          | 1024             | 4      | 512       | 4096           |
| Redis         | 2×6338 / 24×64GB / 2×NVMe32T / 2×25Gb/s        | 128          | 1536             | 4      | 512       | 6144           |
| Kubernetes    | 2×6338 / 8×32GB / 2×NVMe2T / 2×25Gb/s          | 128          | 256              | 3      | 384       | 768            |
| NGINX         | 2×6338 / 4×32GB / 1×NVMe1T / 2×25Gb/s          | 128          | 128              | 2      | 256       | 256            |
| Kafka         | 2×6338 / 8×32GB / 2×NVMe1T / 2×25Gb/s          | 128          | 256              | 2      | 256       | 512            |
| Milvus        | 2×6338 / 2×32GB / 4×NVMe4T / 2×25Gb/s          | 128          | 64               | 2      | 256       | 128            |


Kubernetes
| Сервис               | Конфигурация                                       | vCPU на узел | RAM на узел (GB) | Кол-во | Итог vCPU | Итог RAM (GB) |
|----------------------|-----------------------------------------------------|--------------|------------------|--------|-----------|----------------|
| API Gateway          | 1×6338 / 4×32GB / 1×NVMe1T / 2×25Gb/s              | 64           | 128              | 2      | 128       | 256            |
| Prometheus + Grafana | 1×6338 / 4×32GB / 1×NVMe2T / 2×25Gb/s              | 64           | 128              | 1      | 64        | 128            |
| CDN + Edge           | 1×6338 / 2×32GB / 1×NVMe2T / 2×25Gb/s              | 64           | 64               | 3      | 192       | 192            |
| K8s Worker Nodes     | 2×6338 / 8×32GB / 2×NVMe4T / 2×25Gb/s              | 128          | 256              | 6      | 768       | 1536           |
